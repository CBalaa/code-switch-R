...
export async function fetchAllProviderEndpoints(): Promise<SyncedEndpoint[]> {
  ...
  try {
    // 2. 获取 OpenAI Responses 供应商
    const responsesProviders = await LoadProviders('openai-responses')
    if (Array.isArray(responsesProviders)) {
      responsesProviders.forEach((p: any) => {
        if (p.apiUrl && p.apiUrl.trim()) {
          const baseUrl = extractBaseUrl(p.apiUrl)
          if (baseUrl) {
            endpoints.push({
              ...
              providerName: p.name || 'OpenAI Responses Provider'
            })
          }
        }
      })
    ...
    console.error('获取 OpenAI Responses 供应商失败:', error)
  }

  try {
    // 3. 获取 OpenAI Chat 供应商
    const chatProviders = await LoadProviders('openai-chat')
    if (Array.isArray(chatProviders)) {
      chatProviders.forEach((p: any) => {
        if (p.apiUrl && p.apiUrl.trim()) {
          const baseUrl = extractBaseUrl(p.apiUrl)
          if (baseUrl) {
            endpoints.push({
              url: baseUrl,
              source: 'openai-chat',
              providerName: p.name || 'OpenAI Chat Provider'
            })
          }
        }
      })
